...
uint32_t emu_disasm_addr(uc_engine *uc, uint32_t eip, char *str)
{
	INSTRUCTION inst;
	...
	if(uc_mem_read(uc,eip,data,32)!=0){
		snprintf(str, 80, "???? No memory At Address");			
		return 0;
	}
  
	// step 2: fetch instruction-
	instrsize = get_instruction(&inst, data, MODE_32);
	if( instrsize == 0 )
	{
